...
from orchestrator.cli import _parse_plugin_lines, _parse_plugin_spec


class TestParsePluginSpec:
    def test_modrinth_latest(self) -> None:
        spec = _parse_plugin_spec("modrinth:luckperms@latest")
    ...
    def test_empty_params_when_omitted(self) -> None:
        spec = _parse_plugin_spec("github:MilkBowl/Vault@latest")
        assert spec.params == {}

    def test_multiple_params(self) -> None:
        spec = _parse_plugin_spec("github:Foo/Bar[regex=foo.jar,other=val]@latest")
        assert spec.params == {"regex": "foo.jar", "other": "val"}

    def test_params_with_specific_version(self) -> None:
        spec = _parse_plugin_spec("github:Foo/Bar[regex=foo.jar]@1.2.3")
        assert spec.version == "1.2.3"
        assert spec.params == {"regex": "foo.jar"}

    def test_param_accessor_returns_value(self) -> None:
        spec = _parse_plugin_spec("github:Foo/Bar[regex=foo.jar]@latest")
        assert spec.param("regex") == "foo.jar"

    def test_param_accessor_returns_none_for_missing_key(self) -> None:
        spec = _parse_plugin_spec("github:Foo/Bar@latest")
...
class TestParsePluginLines:
    def test_skips_blanks_and_comments(self) -> None:
        specs = _parse_plugin_lines("modrinth:luckperms@latest\n\n# This is a comment\nhangar:libertybans@latest\n  \n")
        assert len(specs) == 2
